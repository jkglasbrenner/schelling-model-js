...
When *Similarity* is set to $30\%$, then an agent will be satisfied if $30\%$ or more of its neighbors are the same color, which is the case in the following diagrams:

![Examples of satisfied agents]()
...
The diagrams below show examples of an unsatisfied agent:

![Examples of unsatisfied agents]()
...
However, you will need to press the `Reset` button if you adjust the parameters that change the starting properties for the grid, such as $n$, the percentage of empty squares, and the blue to orange agent ratio.
